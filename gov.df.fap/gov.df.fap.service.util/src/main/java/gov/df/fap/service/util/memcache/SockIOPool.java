...
public class SockIOPool {
  ...
  // MemCache服务是否正常启动
  public boolean memCacheIsNormalStart() {
    return availPool != null && availPool.size() > 0;
  }
   ...
   * @param path
   */
  public static void startMemcached(String host) {
    try {
      String[] ipAndPort = host.split(":");
      String ip = ipAndPort[0];
      String port = ipAndPort[1];
      boolean flag = serviceIsStart(ip, port);//是否启动
      if (!flag) {
        String sp = System.getProperties().getProperty("file.separator");
        String path = SessionUtil.getWebPath() + sp + "memcach" + sp;
        String OS = System.getProperty("os.name").toLowerCase();
        String userHome = System.getProperty("user.home") + sp + "memcach" + sp;
        if (OS.indexOf("windows") > -1) {
          String fileName = "memcached.exe";
          copyFileToUserHome(path, userHome, fileName);
          String temp = userHome.replaceAll("\\s+", "\" \"");//替换文件路径中的空格
          Runtime.getRuntime().exec("cmd /c " + temp + fileName + " -d install ");
          Runtime.getRuntime().exec("cmd /c " + temp + fileName + " -d start -l " + ip + " -p " + port);
          System.out.println("IP为:" + ip + "端口为:" + port + "的memcached服务在Windows系统中自启动成功");
        } else if (OS.indexOf("linux") > -1) {
          String command = "/usr/local/bin/memcached -d -u root -l " + ip + " -p " + port;
          Runtime.getRuntime().exec(command);
          System.out.println("IP为:" + ip + "端口为:" + port + "的memcached服务在Linux系统中自启动成功");
        } else {
          log.error("memcache自启动时不支持该操作系统,需要手工启动缓存服务");
        }
        Thread.sleep(2000);//暂停一会，等memcached服务启动
      }
    } catch (Exception e) {
      log.error("memcache【" + host + "】启动失败");
      e.printStackTrace();
    }
  }

  public static void copyFileToUserHome(String srcFile, String userHome, String fileName) {
    try {
      ...
      FileUtils.copyFileToDirectory(new File(srcFile + fileName), new File(userHome), false);
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
   ...
   * @return
   */
  private static boolean serviceIsStart(String ip, String port) {
    boolean flag = false;
    URL url = null;
    try {
      ...
      in.close();
      log.info("IP为:" + ip + "端口为:" + port + "的memcached服务已经启动");
      flag = true;
    } catch (Exception e) {
      log.info("IP为:" + ip + "端口为:" + port + "的memcached服务没有启动");
    }
    return flag;
  }
  ...
  private static boolean isUseMemcache = false;
  ...
  public synchronized void initialize() {
    ...
    if (isUseMemcache) {// 使用分布式緩存
      ...
      maxCreate = (poolMultiplier > minConn) ? minConn : minConn / poolMultiplier; // only create up to maxCreate connections
      // at
      // once
  ...
  protected SockIO createSocket(String host) {
    ...
    try {
      ...
      log.error("检查一下memcache缓存服务是否正常启动");
      log.error(ex.getMessage(), ex);
      socket = null;
    }
  ...
  public static class SockIO {
    ...
    public SockIO(SockIOPool pool, String host, int port, int timeout, int connectTimeout, boolean noDelay)
      ...
      sock = (connectTimeout > 0) ? getSocket(host, port, connectTimeout) : new Socket(host, port);
     ...
     *             if hostname is invalid
     */
    public SockIO(SockIOPool pool, String host, int timeout, int connectTimeout, boolean noDelay) throws IOException,
      ...
      sock = (connectTimeout > 0) ? getSocket(ip[0], Integer.parseInt(ip[1]), connectTimeout) : new Socket(ip[0],
    ...
    void trueClose() throws IOException {
      ...
      if (out != null) {
        try {
          ...
          log.error(ioe.getMessage(), ioe);
          errMsg
            .append("++++ error closing output stream for socket: " + toString() + " for host: " + getHost() + "\n");
          errMsg.append(ioe.getMessage());
          err = true;
        }
      }
