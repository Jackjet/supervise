...
public abstract class BaseBpmnJsonConverter implements EditorJsonConstants, StencilConstants {
    ...
    public void convertToJson(BaseElement baseElement, ActivityProcessor processor, BpmnModel model, FlowElementsContainer container, ArrayNode shapesArrayNode, double subProcessX, double subProcessY) {
        ...
        //begin_加入跨节点_2017-6-1 6-5
        if (baseElement instanceof FlowJumpNode) {
          FlowJumpNode flowJumpNode = (FlowJumpNode) baseElement;
          for (SequenceJumpFlow sequenceJumpFlow : flowJumpNode.getOutgoingFlowsOfJump()) {
              outgoingArrayNode.add(BpmnJsonConverterUtil.createResourceNode(sequenceJumpFlow.getId()));
          }
      }
        //end_加入跨节点_2017-6-1
        ...
        if(baseElement instanceof FlowElement) {
        	FlowElement flowElement = (FlowElement) baseElement;
        	BpmnJsonConverterUtil.convertListenersToJson(flowElement.getExecutionListeners(), true, propertiesNode);
        }
        if (baseElement instanceof Activity) {
            ...
            if (activity.isAsynchronous()) {
                propertiesNode.put(PROPERTY_ASYNCHRONOUS, PROPERTY_VALUE_TRUE);
            }else
            	propertiesNode.put(PROPERTY_ASYNCHRONOUS, "false");

            if (activity.isNotExclusive()) {
                propertiesNode.put(PROPERTY_EXCLUSIVE, "false");
            }else{
            	propertiesNode.put(PROPERTY_EXCLUSIVE, "true");
            }
        ...
        for (Artifact artifact : model.getProcesses().get(0).getArtifacts()) {
    ...
    public void convertToBpmnModel(JsonNode elementNode, JsonNode modelNode, ActivityProcessor processor, BaseElement parentElement, 
        ...
        if (baseElement instanceof FlowElement) {
            ...
            if (baseElement instanceof Activity) {
                ...
                activity.setForCompensation(getPropertyValueAsBoolean(PROPERTY_COMPEMSATE, elementNode));
                ...
                if (activity.getLoopCharacteristics()==null&& StringUtils.isNotEmpty(multiInstanceType) && "none".equalsIgnoreCase(multiInstanceType) == false) {
        ...
        if (baseElement instanceof FlowElement) {
            ...
            //begin_加入跨节点_2016-6-1
            if (flowElement instanceof SequenceJumpFlow) {
              ExtensionElement idExtensionElement = new ExtensionElement();
              idExtensionElement.setName("EDITOR_RESOURCEID");
              idExtensionElement.setElementText(elementNode.get(EDITOR_SHAPE_ID).asText());
              flowElement.addExtensionElement(idExtensionElement);
    ...
    protected void convertJsonToTimerDefinition(JsonNode objectNode, Event event) {
        ...
        String timeDateParam = getPropertyValueAsString(PROPERTY_TIMER_DATE_PARAM, objectNode);
        ...
        if(StringUtils.isNotEmpty(timeDateParam)){
        	eventDefinition.setTimeDate(timeDateParam);
        } else if (StringUtils.isNotEmpty(timeDate)) {
    ...
    protected void convertJsonToTerminateDefinition(JsonNode objectNode, Event event) {
        //String errorRef = getPropertyValueAsString(PROPERTY_ERRORREF, objectNode);
    ...
    protected void convertJsonToCompensateDefinition(JsonNode objectNode, Event event) {
	    String activityRef = getPropertyValueAsString(PROPERTY_COMPENSATE_ACTIVITYREF, objectNode);
	    Boolean waitForCompletion = getPropertyValueAsBoolean(PROPERTY_COMPENSATE_WAITFORCOMPLETION, objectNode);
	    ...
	    if (StringUtils.isNotEmpty(activityRef)) {
	      ...
	      eventDefinition.setWaitForCompletion(waitForCompletion);
	    }
	    event.getEventDefinitions().add(eventDefinition);
	  }
