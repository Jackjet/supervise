...
public class CallActivityJsonConverter extends BaseBpmnJsonConverter {
  ...
  protected FlowElement convertJsonToElement(JsonNode elementNode, JsonNode modelNode, Map<String, JsonNode> shapeMap) {
    ...
    if(callActivity.getCalledElement() == null){
    	JsonNode calledElementNode = getProperty(PROPERTY_CALLACTIVITY_CALLEDELEMENT, elementNode);
    	if(calledElementNode != null){
    		JsonNode calledElementDefNode = calledElementNode.get("refResultData");
            if (calledElementDefNode != null && !calledElementDefNode.isNull()) {
            	JsonNode processId = calledElementDefNode.get(0).get("code");
            	if(processId != null && !processId.isNull()){
            		callActivity.setCalledElement(processId.asText());
            	}
            }
    	}
    }
