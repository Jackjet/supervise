...
require(['jquery', 'knockout', 'bootstrap', 'uui', 'tree', 'grid', 'ip','select2'], function ($, ko) {
  ...
  var msgContentFields = [
    ...
    {key_name: '接受人', value_name: 'toUser', inset: true},
  ...
  var tmpl_list = {
    nodes: [
      {name: '所有', children: [{name: 'abc'},{name: 'def'},{name: 'ade'}]}
    ],
    instance: null,
    setting: {},
    sel_index: 0,
  ...
  function beforeClick(treeId, treeNode) {
    if(!treeNode.level || treeNode.children && treeNode.children.length) {
      return false
    }
  }
          ...
          $.ajax({
            ...
            success: function(data) {
              if(data.result === 'success') {
                ...
                vm.convertUserToStr(data.hasSelectedUsers, data.hasSelectedGroup)
              }
            },
            error: function() {
              ip.ipInfoJump('获取节点失败', 'error')
          ...
          function _init(list) {
            ...
            if(list[0]['send_type']) {
              list[0]['send_type'].split('').forEach(function(flag, i) {
                flag = flag != 0
                switch(i) {
                  case 0: vm.send_type.sys(flag); break
                  case 1: vm.send_type.app(flag); break
                  case 2: vm.send_type.info(flag); break
                  case 3: vm.send_type.wechat(flag); break
                }
              })
            }
          }
  ...
  window.vm = {
    ...
    addField: function() {
      ...
      if(list.some(function(o) {return o.key_name === this.key_name().trim()}.bind(this))) {
    ...
    setNode: function() {
      ...
      var self = this
      var obj = wfNodes.filter(function(o) {return o.chr_id === self.msg.wf_node_id()})[0]
    ...
    selUser: function(treeId) {
      var tree_left = this.is_user ? zTree.getZTreeObj('user-tree1') : zTree.getZTreeObj('users-group1')
      ...
      if(this.is_user) { // 添加用户
        var users = tree_right.getNodesByParam('name', '用户', rightNodes[0])[0]
        if(!users) {
          users = tree_right.addNodes(rightNodes[0], {name: '用户', children: [nodes[0]], open: true})
        } else
          tree_right.addNodes(users, nodes[0])
      } else { // 添加用户群
        users = tree_right.getNodesByParam('name', '用户群', rightNodes[0])[0]
        if(!users) {
          users = tree_right.addNodes(rightNodes[0], {name: '用户群', children: [nodes[0]], open: true})
        } else
          tree_right.addNodes(users, nodes[0])
      }
    },
    selAllUser: function() {
      ...
      var rightNodes = tree_right.getNodes()
      if(this.is_user) { // 添加用户
        var users = tree_right.getNodeByParam('name', '用户', rightNodes[0])
        if(!users) {
    ...
    saveUser: function() {
      ...
      this.convertUserToStr(users, userGroup)

    },
    convertUserToStr: function(users, userGroup) {
      var str = ''
      var str1 = ''
      // 获取用户群
      if(userGroup && userGroup.length) {
        str = '用户群' + '-'+userGroup.map(function(o) {return o.name}).join(',')
        str1 = '用户群' + '-'+userGroup.map(function(o) {return o.id}).join(',')
      }
      if(users && users.length) {
        str = str && str + ';'
        str1 = str1 && str1 + ';'
        str += '用户' + '-'+users.map(function(o) {return o.name}).join(',')
        str1 += '用户' + '-'+users.map(function(o) {return o.id}).join(',')
      }
      this.receiver(str)
      this.receiver1(str1)
    },
    ...
    saveUserGroup: function() {
      ...
      var userIdStr = nodes.map(function(o) {return o.id}).join('#')
      var data = {
        groupName: groupName,
        groupId: groupId,
        userIdStr: userIdStr
  ...
  function collectWf() {
    ...
    var obj = wfList.filter(function(o) {return o.chr_id === flow})[0]
    this.msg.wf_flow_id(flow)
    this.msg.wf_flow_code(obj.chr_code)
    this.msg.wf_flow_name(obj.chr_name)

    // 设置wf_node_name  wf_node_code
    obj = wfNodes.filter(function(o) {return o.chr_id === node})[0]
  ...
  function valid() {
    ...
    if(vm.msg_type === 'new' && msgRules.some(function(o) {if(o.code === msg.msg_rule_code) return true})) {
  ...
  function fetchTmpl() {
    ...
    function initTree(list) {
      ...
      tree11.nodes = nodes
      tree11.instance =
        $.fn.zTree.init($('#tmpl-list'),tree11.setting, nodes)
