			...
			if(isChecked == 0)
				{
					...
					if(obj == null)
						{
							ip.ipInfoJump("选择部分权限必须至少勾选一个要素！");
							return;
						}
					right_type = 1 ;
			...
			$.ajax({
				...
				success: function (data) {
					if(data.result == "success"){
						...
						ip.ipInfoJumpump("删除失败！"+ data.reason);
					}
				}
			});
		}
