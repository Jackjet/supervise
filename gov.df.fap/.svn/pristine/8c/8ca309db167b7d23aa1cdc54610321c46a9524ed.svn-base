...
//        		var officeList = nodes;
        		...
        		$.each(nodes, function(index, item){
                    var obj = {};
                    obj.CHR_ID = item.CHR_ID;
                    obj.CHR_CODE = item.CHR_CODE;
                    obj.CHR_NAME = item.CHR_NAME;
                    officeList.push(obj);
                });
        	}else if(OS == "1"){
        		var nodes = treeObj.getNodes();
			...
			if(flag){
				...
				var taskGetList2 = [];
				$.each(taskGetList, function(index, item){
                    var obj = {};
                    obj.CHR_ID = item.CHR_ID || item.chr_id;
                    obj.CHR_CODE = item.CHR_CODE || item.chr_code;
                    obj.CHR_NAME = item.CHR_NAME || item.chr_name;
                    taskGetList2.push(obj);
                });
				var workFlowMap = {
						"menu_id":menuId,
						"billtype_code":billtype_code,
       		    ...
       		    var data = {
       		    	...
       		    	"supList":JSON.stringify(taskGetList2),
       		    	"billtype_code":billtype_code,
