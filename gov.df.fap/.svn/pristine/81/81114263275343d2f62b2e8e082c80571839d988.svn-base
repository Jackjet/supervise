...
import gov.df.fap.service.util.wf.activiti.ext.FlowJumpNode;
import gov.df.fap.service.util.wf.activiti.ext.SequenceJumpFlow;
import gov.df.fap.service.util.wf.activiti.json.converter.AssociationJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.BoundaryEventJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.BusinessRuleTaskJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.CallActivityJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.CamelTaskJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.CatchEventJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.DataObjectJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.EndEventJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.EventGatewayJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.EventSubProcessJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.ExclusiveGatewayJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.InclusiveGatewayJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.MailTaskJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.ManualTaskJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.MuleTaskJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.ParallelGatewayJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.ReceiveTaskJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.ScriptTaskJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.SendTaskJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.SequenceFlowJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.SequenceJumpFlowJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.ServiceTaskJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.StartEventJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.SubProcessJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.TextAnnotationJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.ThrowEventJsonConverter;
import gov.df.fap.service.util.wf.activiti.json.converter.UserTaskJsonConverter;
...
public class BpmnJsonConverter implements EditorJsonConstants, StencilConstants, ActivityProcessor {
    ...
    static {
        ...
        SequenceJumpFlowJsonConverter.fillTypes(convertersToBpmnMap, convertersToJsonMap);//加入跨节点_2017-6-1
        ...
        ApproveUserTaskJsonConverter.fillTypes(convertersToBpmnMap, convertersToJsonMap);
        ...
        DataObjectJsonConverter.fillTypes(convertersToBpmnMap, convertersToJsonMap);
        
        /*supportedElementClasses.add(ApproveUserTask.class);
        getDiRectangles().add("ApproveUserTask");*/
        
      /*  try {
          FlowJumpNode newInstance = FlowJumpNode.class.newInstance();
    ...
    static {
        ...
        DI_CIRCLES.add(STENCIL_EVENT_THROW_COMPENSATE);
    ...
    public ObjectNode convertToJson(BpmnModel model) {
        ...
        if (CollectionUtils.isNotEmpty(mainProcess.getCandidateStarterUsers())) {
        	ObjectNode candidateNode=BpmnJsonConverterUtil.convertRefValue(mainProcess.getCandidateStarterUsers());
        	propertiesNode.put("candidatestarterusers", candidateNode);
        }
            ...
            for (Pool pool : model.getPools()) {
                ...
                if (process != null) {
                    ...
                    for (FlowElement flowElement : process.getFlowElements()) {
                        ...
                        } else if (flowElement instanceof SequenceJumpFlow) {//begin_加入跨节点_2017-6-1
                          SequenceJumpFlow sequenceJumpFlow = (SequenceJumpFlow) flowElement;
                          lookForElement = model.getFlowElement(sequenceJumpFlow.getSourceRef());
                        ...
                        //begin_加入跨节点_2017-6-1
                        if (flowElement instanceof SequenceJumpFlow || laneForElement != null) { 
                          processFlowElement(flowElement, process, model, laneMap.get(laneForElement.getId()), 
                                  laneGraphicInfo.getX(), laneGraphicInfo.getY());
                      }
   ...
   /* protected void processMessageFlows(BpmnModel model, ArrayNode shapesArrayNode) {
    ...
    public BpmnModel convertToBpmnModel(JsonNode modelNode) {
        ...
        if (nonEmptyPoolFound == false) {
            ...
            JsonNode candidateStarterUsersNode = JsonConverterUtil.getProperty("candidatestarterusers", modelNode);
            if (candidateStarterUsersNode != null) {
                JsonNode candidateusersDefNode = candidateStarterUsersNode.get("refResultData");
                if (candidateusersDefNode != null) {
                    ...
                    process.setCandidateStarterUsers(candidateUsersList);
                    //process.setCandidateStarterUsersName(candidateUsersNameList);
                }
            }
            ...
            for (JsonNode shapeNode : shapesArrayNode) {
                if (STENCIL_SEQUENCE_FLOW.equalsIgnoreCase(BpmnJsonConverterUtil.getStencilId(shapeNode)) ||
                    ...
                    if (sourceRef != null) {
                        ...
                        SequenceJumpFlowJsonConverter jumpFlowConverter = new SequenceJumpFlowJsonConverter();
                        if (lane != null) {
                        ...
                        //begin_加入跨节点_2017-6-1
                            if (lane != null) {
                              jumpFlowConverter.convertToBpmnModel(shapeNode, modelNode, this, lane, shapeMap, bpmnModel);
                          } else {
                            jumpFlowConverter.convertToBpmnModel(shapeNode, modelNode, this, bpmnModel.getProcesses().get(0), shapeMap, bpmnModel);
                          }
                        //end_加入跨节点_2017-6-1
        ...
        for (Process process : bpmnModel.getProcesses()) {
            ...
            if (subShapesMap.size() > 0) {
                ...
                List<String> removeSubFlowsOfJumpList = new ArrayList<String>();
                ...
                //begin_加入跨节点_2017-6-1
                    for (FlowElement flowElement : process.findFlowElementsOfType(SequenceJumpFlow.class)) {
                      SequenceJumpFlow sequenceJumpFlow = (SequenceJumpFlow) flowElement;
                      if (subShapesMap.containsKey(sequenceJumpFlow.getSourceRef())) {
                          SubProcess subProcess = subShapesMap.get(sequenceJumpFlow.getSourceRef());
                          if (subProcess.getFlowElement(sequenceJumpFlow.getId()) == null) {
                            subProcess.addFlowElement(sequenceJumpFlow);
                            removeSubFlowsOfJumpList.add(sequenceJumpFlow.getId());
                          }
                      }
                  }
                    
                    for (String flowId : removeSubFlowsOfJumpList) {
                      process.removeFlowElement(flowId);
                  }
    ...
    private void postProcessElements(FlowElementsContainer parentContainer,  Collection<FlowElement> flowElementList, 
          ...
          //begin_2017-6-6
    /*  Class<FlowJumpNode> newInstanceOfJump=null;
      // newInstanceOfJump = FlowJumpNode.class.newInstance();
       try {
        newInstanceOfJump=(Class<FlowJumpNode>) Class.forName("gov.df.fap.service.util.wf.activiti.ext.FlowJumpNode");
      } catch (ClassNotFoundException e1) {
        // TODO Auto-generated catch block
        e1.printStackTrace();
      }*/
      FlowJumpNode flowJumpNode = new FlowJumpNode();
          //end_2017-6-6
        ...
        for (FlowElement flowElement : flowElementList) {
            ...
            }else if (flowElement instanceof SequenceJumpFlow) {//begin_加入跨节点_2017-6-1  6-5
              SequenceJumpFlow sequenceJumpFlow = (SequenceJumpFlow) flowElement;
              FlowElement sourceFlowElement = parentContainer.getFlowElement(sequenceJumpFlow.getSourceRef());
              if (sourceFlowElement != null && sourceFlowElement instanceof FlowNode) {
                  
                  FlowWithContainer flowWithContainer = new FlowWithContainer(sequenceJumpFlow, parentContainer);
                  if (sequenceJumpFlow.getExtensionElements().get("EDITOR_RESOURCEID") != null && sequenceJumpFlow.getExtensionElements().get("EDITOR_RESOURCEID").size() > 0) {
                      allFlowMap.put(sequenceJumpFlow.getExtensionElements().get("EDITOR_RESOURCEID").get(0).getElementText(), flowWithContainer);
                      sequenceJumpFlow.getExtensionElements().remove("EDITOR_RESOURCEID");
                  }
                  
                  flowJumpNode.getOutgoingFlowsOfJump().add(sequenceJumpFlow);
                  
                
                  JsonNode edgeNode = edgeMap.get(sequenceJumpFlow.getId());
                  if (edgeNode != null) {
                    boolean isDefault = JsonConverterUtil.getPropertyValueAsBoolean("defaultflow", edgeNode);
                    if (isDefault) {
                        if (sourceFlowElement instanceof Activity) {
                            ((Activity) sourceFlowElement).setDefaultFlow(sequenceJumpFlow.getId());
                        } else if (sourceFlowElement instanceof Gateway) {
                            ((Gateway) sourceFlowElement).setDefaultFlow(sequenceJumpFlow.getId());
                        }
                    }
                  }
              }
              FlowElement targetFlowElement = parentContainer.getFlowElement(sequenceJumpFlow.getTargetRef());
              if (targetFlowElement != null && targetFlowElement instanceof FlowJumpNode) {
                  ((FlowJumpNode) targetFlowElement).getIncomingFlowsOfJump().add(sequenceJumpFlow);
              }
          }//end_加入跨节点_2017-6-1
        }
    }
    ...
    private void readEdgeDI(Map<String, JsonNode> edgeMap, Map<String, List<JsonNode>> sourceAndTargetMap, BpmnModel bpmnModel) {
        ...
        for (String edgeId : edgeMap.keySet()) {
			...
			try {
				firstLine = new Line2D(sourceRefLineX, sourceRefLineY, nextPointInLineX, nextPointInLineY);
			} catch (Exception e) {
				e.printStackTrace();
			}
    ...
    class FlowWithContainer {
        ...
        protected SequenceJumpFlow sequenceJumpFlow;//加入跨节点_2017-6-1
        ...
        //begin_加入跨节点-2017-6-1
            public FlowWithContainer(SequenceJumpFlow sequenceJumpFlow, FlowElementsContainer flowContainer) {
              this.sequenceJumpFlow = sequenceJumpFlow;
              this.flowContainer = flowContainer;
          }
        //end_加入跨节点-2017-6-1
        ...
        public SequenceJumpFlow getSequenceJumpFlow() {
          return sequenceJumpFlow;
        }

        public void setSequenceJumpFlow(SequenceJumpFlow sequenceJumpFlow) {
          this.sequenceJumpFlow = sequenceJumpFlow;
        }
	...
	public static void setConvertersToJsonMap(
			Map<Class<? extends BaseElement>, Class<? extends BaseBpmnJsonConverter>> convertersToJsonMap) {
		BpmnJsonConverter.convertersToJsonMap = convertersToJsonMap;
	}
	...
	public static void setConvertersToBpmnMap(
			Map<String, Class<? extends BaseBpmnJsonConverter>> convertersToBpmnMap) {
		BpmnJsonConverter.convertersToBpmnMap = convertersToBpmnMap;
	}

	public static List<String> getDiRectangles() {
		return DI_RECTANGLES;
	}
