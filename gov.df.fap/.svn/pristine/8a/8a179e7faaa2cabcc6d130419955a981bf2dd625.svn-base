						...
						if(parentid == "0" ){
							...
							$.ajax({
								...
								success: function (data) {
									...
									menuViewModel.gridDataTable1.setSimpleData(data.eledetail);
...
//											var gridModle =ip.initGrid(data, "elegrid", "/df/elementConfig/getElementSourceGrid.do?tokenid="+tokenid, options, 1,false,false,true);
	...
	menuViewModel.delbtn = function (){
		...
		$("#sid").on("click",function(){
			...
			$.ajax({
				...
				type: 'POST',
				dataType: 'json',
				data: {"chr_id":chr_id,"ajax":"noCache","tablename":tablename},
				success: function (data) {
					var flag = data.flag;
					if(flag == "1"){
	...
	menuViewModel.editviewSubmit = function(){
		...
		$.ajax({
			...
			success: function (data) {
				...
				$("#num").val(num);
				menuViewModel.gridDataTable3.removeAllRows();
				menuViewModel.gridDataTable3.setSimpleData(tableDate);
	...
	menuViewModel.ensure = function (){
		...
		$.ajax({
			...
			type: 'POST',
			dataType: 'json',
			data: {"upobj":upobj,"ajax":"noCache","tablename":tablename,"pid":pid},
			success: function (data) {
				var flag = data.flag;
				if(flag == "1"){
	...
	menuViewModel.ensure2 = function (){
		...
		$.ajax({
			...
			type: 'POST',
			dataType: 'json',
			data: {"upobj":upobj,"ajax":"noCache","tablename":tablename,"chr_id":chr_id,"pid":pid,"oldpid":oldpid},
			success: function (data) {
				var flag = data.flag;
				if(flag == "1"){
	...
	menuViewModel.initGrid = function(creatData,viewid,areaid,url,options) {
		...
		viewModel.createGrid = function(data) {
			...
			var innerHTML = "<div u-meta='" + '{"id":"' + viewId + '","data":"gridData","type":"grid","editable":false,"autoExpand":false,"showTree":false,"showNumCol":true,"keyField":"theId","parentKeyField":"parentId","multiSelect":true,"height":500}' + "'>";
			for(var i = 0; i < creatData.length; i++ ) {
