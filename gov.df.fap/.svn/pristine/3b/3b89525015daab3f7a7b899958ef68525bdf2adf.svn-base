require(['jquery', 'knockout', 'bootstrap', 'uui','tree','grid','director','ip'], function ($, ko) {
	...
	menuViewModel.ensure = function (){
		...
		if(role_code ==""||role_code==null){
			ip.ipInfoJump("角色编码不可为空！","success");
			return;
		}
		if(role_name ==""||role_code==null){
			ip.ipInfoJump("角色名称不可为空！","success");
			return;
		}
		...
		$.ajax({
			...
			type: 'GET',
			dataType: 'json',
			data: {"role_code":role_code,"role_name":role_name,"user_sys_id":user_sys_id,"enabled":enabled,"ajax":"noCache"},
			success: function (data) {
				if(data.flag == "1"){
