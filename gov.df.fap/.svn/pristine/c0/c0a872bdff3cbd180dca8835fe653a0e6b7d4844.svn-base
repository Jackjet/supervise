                        	...
                        	$.ajax({
                                ...
                                success: function (result) { 
                                    if(result.errorCode == 0){
                                        ...
                                        for(var n=0;n<btnData.length;n++) {
                                    		var btnId = btnData[n].BUTTON_ID;
                                    		if((btnData[n].DISPLAY_STATUS == "9" && btnData[n].ENABLED == "1") || btnData[n].DISPLAY_STATUS == "1") {
                                    			$("#"+btnId).removeAttr("disabled","disabled");
                                    		}else {
                                    			$("#"+btnId).attr("disabled","disabled");
                                    		}
      ...
      //初始化部门树
        viewModel.initDepTree = function () {
        	$.ajax({
	            ...
	            data: {"ele_code":eleCode,"task_id":task_id,"sid":sid,"is_allobj":IS_ALLOBJ,"obj_type_id":OBJ_TYPE_ID,},
	            dataType: 'JSON',
	            async: false,
	            success: function (result) {
