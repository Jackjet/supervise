require(
		[ 'jquery', 'knockout', 'bootstrap', 'uui', 'director', 'tree', 'grid', 'ip' ],
		function($, ko) {
			var menuViewModel = {
				data : ko.observable({}),
			};
			menuViewModel.getInitData = function() {
				...
				$.ajax({
					...
					success : function(data) {
						...
						menuViewModel.initMenu(data.mapMenu.child);
					}
				});
			};
						...
						for (var k = 0; k < child_data_two.length; k++) {
							...
							if (child_data_three.length > 0) {
								for (var j = 0; j < child_data_three.length; j++) {
									menu_template += '<a href="' + child_data_three[j].url + '?tokenid=' + token_id + '">' + child_data_three[j].name + '</a>';
								}
							}
							menu_template += '</dd></dl>';
						}
