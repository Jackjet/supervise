...
ip.getCommonOptions = function(options) {
	...
	options["svOfficeId"] = common_data.svOfficeId;
	options["svOfficeName"] = common_data.svOfficeName;
	options["svOfficeCode"] = common_data.svOfficeCode;
	options["svDepId"] = common_data.svDepId;
	options["svDepName"] = common_data.svDepName;
	options["svDepCode"] = common_data.svDepCode;
	return options;
};
// 获取当前菜单id
...
ip.coaRelation = function(ele_code, coa_id, eleRelations, flag, areaId, viewModel, ele_name, ele_value,condition,parentFlag) {
	...
	$.ajax({
		...
		success: function(data) {
			ip.treeChoice(ele_code, data.eleDetail, flag, viewModel, areaId, ele_name,parentFlag);
		}
	});
};
...
ip.creatMoreHeadInfo = function(more_head_array) {
    ...
    // editFlag:是否可编辑true or false
ip.createGrid = function(viewId, areaId, url, options, flag, operateFlag, selectFlag, pageFlag, sumRowFlag,editFlag) {
	...
	$.ajax({
		...
		success: function(data) {
			view = ip.initGrid(data, areaId, url, options, flag, operateFlag, selectFlag, pageFlag, sumRowFlag,editFlag);
		}
	});
	return view;
}
//带回调处理
ip.initGridWithCallBack = function(callBackFunc,data, areaId, url, options, flag, operateFlag, selectFlag, pageFlag, sumRowFlag,editFlag){
	$("#ip-grid-footer-area-sum-budgetGridArea").html(""); //清空切换状态时候左下角表格金额选中显示的数据
		...
		viewModel.createGrid = function(data) {
			...
			if (sumRowFlag) {
				innerHTML = "<div u-meta='" + '{"id":"' + viewId + '","data":"gridData","type":"grid","noScroll":false,"editType":"'+editType+'","editable":' + editFlag + ',"onValueChange":"' + areaId + '_onValueChange","onRowSelected":"' + areaId + '_onRowSelected","onRowUnSelected":"' + areaId + '_onRowSelected","autoExpand":false,"needLocalStorage":true,"afterCreate": "' + areaId + '_afterCreate","multiSelect": ' + selectFlag + ',"onDblClickFun":"' + areaId + '_onDbClick","showNumCol": true,"showSumRow": true,"sumRowFirst":true,"sumRowFixed": true,"headerHeight":32,"rowHeight":32,"sumRowHeight":32,"cancelFocus":false,"onBeforeRowSelected":"' + areaId + '_onBeforeRowSelected","maxHeaderLevel":' + data.mate.maxHeaderLevel + ',"sortable":true}' + "'>";
			} else {
				innerHTML = "<div u-meta='" + '{"id":"' + viewId + '","data":"gridData","type":"grid","noScroll":false,"editType":"'+editType+'","editable":' + editFlag + ',"onValueChange":"' + areaId + '_onValueChange","onRowSelected":"' + areaId + '_onRowSelected","onRowUnSelected":"' + areaId + '_onRowSelected","autoExpand":false,"needLocalStorage":true,"afterCreate": "' + areaId + '_afterCreate","multiSelect": ' + selectFlag + ',"onDblClickFun":"' + areaId + '_onDbClick","showNumCol": true,"headerHeight":32,"rowHeight":32,"cancelFocus":false,"onBeforeRowSelected":"' + areaId + '_onBeforeRowSelected","maxHeaderLevel":' + data.mate.maxHeaderLevel + ',"sortable":true}' + "'>";
			}
			...
			if (operateFlag) {
				innerHTML += "<div options='" + '{"field":"operate","visible":true,"dataType":"String","editType":"string","title":"操作","fixed":true,"width": 80,"renderType":"' + areaId + '"}' + "'></div>";
			}
			...
			for (var i = 0; i < data.header.length; i++) {
				...
				if (data.header[i].sumflag == "true") {
					...
					}  else if(data.header[i].disp_mode == "datetime") {
						innerHTML += "<div options='" + '{"field":"' + data.header[i].id + '","editType":"string","visible":' + data.header[i].visible + ',"canVisible":' + data.header[i].visible + ',"dataType":"time","title":"' + data.header[i].name + '","headerLevel":"' + data.header[i].headerLevel + '","width": ' + data.header[i].width + ',"sumCol":true,"sumRenderType":"summ","renderType":"' + areaId + '_render"}' + "'></div>";
					}else {
					...
					if (data.header[i].disp_mode == "decimal") {
						...
						innerHTML += "<div options='" + '{"field":"' + data.header[i].id + '","editType":"string","visible":' + data.header[i].visible + ',"canVisible":' + data.header[i].visible + ',"dataType":"time","title":"' + data.header[i].name + '","headerLevel":"' + data.header[i].headerLevel + '","renderType":"' + areaId + '_render","width": ' + data.header[i].width + '}' + "'></div>";
					}else {
			...
			if (pageFlag) {
				innerHTML += "<div id='pagination' style='float: right;' class='u-pagination' u-meta='" + '{"type":"pagination","data":"gridData","pageList":[10,15,20,50,100],"sizeChange":"sizeChangeFun","pageChange":"pageChangeFun"}' + "'></div>";
			}
			innerHTML += "</div>";
		...
		summ = function(obj) {
			...
			obj.element.parentNode.innerHTML = '<div id="summ" class="text-right" style="height:15px; line-height:15px;">' + ip.dealThousands(obj.value) + '</div>';
...
//							ip.warnJumpMsg(data.result, 0, 0, true);
							...
							var totnum = data.totalElements;
							var pagenum = Math.ceil(totnum / 10);
							viewModel.gridData.setSimpleData(data.dataDetail, {
								unSelect: true
							});
							if(pagenum === 0) {
...
ip.initGrid = function(data, areaId, url, options, flag, operateFlag, selectFlag, pageFlag, sumRowFlag,editFlag) {
	$("#ip-grid-footer-area-sum-budgetGridArea").html(""); //清空切换状态时候左下角表格金额选中显示的数据
		var viewModel = {
			gridData: new u.DataTable({
				meta: ''
		...
		viewModel.createGrid = function(data) {
			...
			if (sumRowFlag) {
				innerHTML = "<div u-meta='" + '{"id":"' + viewId + '","data":"gridData","type":"grid","noScroll":false,"editType":"'+editType+'","editable":' + editFlag + ',"onValueChange":"' + areaId + '_onValueChange","onRowSelected":"' + areaId + '_onRowSelected","onRowUnSelected":"' + areaId + '_onRowSelected","autoExpand":false,"needLocalStorage":true,"multiSelect": ' + selectFlag + ',"onDblClickFun":"' + areaId + '_onDbClick","showNumCol": true,"showSumRow": true,"sumRowFirst":true,"sumRowFixed": true,"headerHeight":32,"rowHeight":32,"sumRowHeight":32,"cancelFocus":false,"onBeforeRowSelected":"' + areaId + '_onBeforeRowSelected","maxHeaderLevel":' + data.mate.maxHeaderLevel + ',"sortable":true}' + "'>";
			} else {
				innerHTML = "<div u-meta='" + '{"id":"' + viewId + '","data":"gridData","type":"grid","noScroll":false,"editType":"'+editType+'","editable":' + editFlag + ',"onValueChange":"' + areaId + '_onValueChange","onRowSelected":"' + areaId + '_onRowSelected","onRowUnSelected":"' + areaId + '_onRowSelected","autoExpand":false,"needLocalStorage":true,"multiSelect": ' + selectFlag + ',"onDblClickFun":"' + areaId + '_onDbClick","showNumCol": true,"headerHeight":32,"rowHeight":32,"cancelFocus":false,"onBeforeRowSelected":"' + areaId + '_onBeforeRowSelected","maxHeaderLevel":' + data.mate.maxHeaderLevel + ',"sortable":true}' + "'>";
			}
			if (operateFlag == undefined) {
				operateFlag = true;
			}
			...
			innerHTML += "<div id='ip-grid-footer-area-" + areaId + "' class='text-right' style='height: 36px;'><div id='ip-grid-footer-area-sum-" + areaId + "' class='fl' style='margin: 10px 0 5px 5px;'></div>";
			if (pageFlag) {
				innerHTML += "<div id='pagination' style='float: right;' class='u-pagination' u-meta='" + '{"type":"pagination","data":"gridData","pageList":[10,20,50,100],"sizeChange":"sizeChangeFun","pageChange":"pageChangeFun"}' + "'></div>";
			}
			innerHTML += "</div>";
			$('#' + areaId).append(innerHTML);
		};
...
ip.setGrid = function(viewModel, url, options) {
		...
		$.ajax({
			...
			success: function(data) {
					...
					if (!data.flag) {
						//ip.warnJumpMsg("没有数据", 0, 0, true);
					} else {
						if (data.dataDetail != "") {
							//ip.ipInfoJump("查询成功", "success");
						}
						//总计逻辑：勿删
						// for (var j = 0; j < viewModel.totals.length; j++) {
						// 	viewModel[viewModel.totals[j]] = data.totals[viewModel.totals[j]];
		...
		switch (creatData[i].disp_mode) {
			case "text":
				if (creatData[i].visible) {
					...
					if (creatData[i].editable == "true") {
						html += '<input type="text" class="form-control" id="' + creatData[i].id + '-' + viewId + '" onkeyup="verifyInput(this,\'30\')">';
					} else {
			...
			case "datetime":
				if (creatData[i].visible) {
					...
					if (creatData[i].editable == "true") {
						html += '<input class="form-control"  readonly="readonly" size="16" id="' + creatData[i].id + '-' + viewId + '" type="text" value="">' +
							'<span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>' +
							'<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>';
					} else {
						html += '<input class="form-control" readonly="readonly" size="16" id="' + creatData[i].id + '-' + viewId + '" type="text" value="" disabled>' +
			...
			case "treeassist":
				if (creatData[i].visible) {
					...
					if (creatData[i].editable == "true") {
						html += '<input type="text" readonly="readonly" class="form-control col-md-6 col-sm-6" id="' + creatData[i].id + '-' + viewId + '"  onkeydown="return ip.codeInto(this.id,this.name,0,{},0,this.title,event)">' +
							'<input type="hidden" id="' + creatData[i].id + '-' + viewId + '-h" name="' + creatData[i].source + '">' +
							'<span class="input-group-addon""><span class="glyphicon glyphicon-remove" onclick="ip.clearText(\'' + creatData[i].id + '-' + viewId + '\')"></span></span>' +
							'<span class="input-group-btn" style="padding: 0;">' +
							'<button class="btn btn-default" style="color: #b3a9a9;font-size: 12px;padding: 7px 9px;margin-top: 0px;" type="button" id="' + creatData[i].id + '-' + viewId + '-btn" name="' + creatData[i].source + '" data-toggle="modal" onclick="ip.showAssitTree(this.id,this.name,0,{},0,this.title)"><span class="glyphicon glyphicon-list" aria-hidden="true"></span></button>';
					} else {
						html += '<input type="text" readonly="readonly" class="form-control col-md-6 col-sm-6" id="' + creatData[i].id + '-' + viewId + '"  onkeydown="return ip.codeInto(this.id,this.name,event)" disabled>' +
							'<input type="hidden" id="' + creatData[i].id + '-' + viewId + '-h" name="' + creatData[i].source + '" disabled>' +
							'<span class="input-group-btn" style="padding: 0;">' +
							'<button class="btn btn-default" style="color: #b3a9a9;font-size: 12px;padding: 7px 9px;margin-top: 0px;" type="button" id="' + creatData[i].id + '-' + viewId + '-btn" name="' + creatData[i].source + '" data-toggle="modal" disabled><span class="glyphicon glyphicon-list" aria-hidden="true"></span></button>';
					}
					html += '</span>' +
						'</div>' +
						'</div>';
			...
			case "textarea":
				if (creatData[i].visible) {
					...
					'<label class="col-md-2 col-sm-2 text-right" style="padding-right: 33px;">' + creatData[i].name + '</label>' +
					'<div class="input-group col-md-10 col-sm-10 modal-input-group">';
				if (creatData[i].editable == "true") {
					html += '<textarea id="' + creatData[i].id + '-' + viewId + '" style="width:100%" onkeyup="verifyInput(this,\'100\')"></textarea>';
				} else {
					html += '<textarea id="' + creatData[i].id + '-' + viewId + '" disabled style="width:100%"></textarea>';
				}
				html += '</span>' +
					'</div>' +
					'</div>';
...
ip.codeInto = function(id, element, flag, viewModel, areaId, ele_name,parentFlag, e) {
		...
		$.ajax({
			...
			success: function(data) {
				if (data.eleDetail.length > 1) {
					ip.treeChoice(id, data.eleDetail, flag, viewModel, areaId, ele_name,parentFlag);
					// $("#" + id + "-btn").click();
				} else {
...
ip.tree_viewModel = {};
ip.areaId = "";
ip.treeChoice = function(id, data, flag, viewModel, areaId, ele_name, parentFlag,callBack) {
	if ( parentFlag ) {
		ip.choice_parent_flag = parentFlag;
	}
		...
		if ( select_node[0].isParent && ip.choice_parent_flag) {
			...
			for (var i = 0; i < select_node.length; i++) {
				...
				if (i == select_node.length - 1) {
					...
					tree_string += select_node[i].id + "@" + encodeURI(select_node[i].chr_name, "utf-8") + "@" + select_node[i].chr_code + "@" + select_node[i].pId;
				} else {
					tree_string_old += select_node[i].codename + ",";
 ...
 *  颜色
*/
ip.highLightKeyWord = function(areaId,key, color) {  
    var keyWord = key || "";  
    var keyColor = color || "red";  
    var nodes =$("#"+areaId).find('.u-grid-content-td-div');//可使用document.getElementById替换  
    if(nodes.length != 0){
...
ip.fuzzyQuery = function(data,id,viewModel) {
	var dataTable = viewModel.gridData;
	var areaId = viewModel.areaid;
    ...
    for (var i = 0; i < data.length; i++) {
    	...
    	for (var j = 0; j < curGridHead.length; j++) {
    		var value = data[i][curGridHead[j]]||data[i][curGridHead[j].toLowerCase()];
    		if(value){
        		if (value.toString().indexOf(search_text) !== -1) {
                    result.push(data[i]);
                    break;
                }
        	}
...
/*        for (item in data[i]) {
	...
	ip.highLightKeyWord(areaId,search_text, "red");
}
//给所有弹窗添加拖动功能
