			...
			uiConfigureViewModel.getInitData = function() {
				...
				$.ajax({
					...
					success : function(data) {
						uiConfigureViewModel.treeDataTable.setSimpleData(data.viewlist);
						if(node != undefined){
			...
			uiConfigureViewModel.baseSaveButton = function(){
		    	...
		    	var viewData= {
		    	 ...
		    	 $.ajax({
						...
						type : 'POST',
						dataType : 'json',
						data:{"viewData":JSON.stringify(viewData),"optType":optType,"tokenId":tokenid,"ajax":"nocache"},
						success : function(data) {
							if(data.flg == "sucess"){
								$("#addModal").modal('hide');
								var ztree = $.fn.zTree.getZTreeObj("uiConfigureTree");
