require(['jquery', 'knockout','bootstrap', 'uui','tree','grid','director','ip'], function ($, ko) {
				...
				$.ajax({
					...
					success: function(data){
						...
						if(data.org_type != null || data.orgTypes != null && data.orgTypes.length > 0){
							viewModel.organRightDataTable.setSimpleData(data.orgTypes);
    			...
    			if(data != null){
    				...
    				for(var i=0; i<data.users.length; i++){
    					...
    					if(data.users[i].code != null){
    						dataItem["name"] = data.users[i].code+" "+ data.users[i].name;
    					}else{
    						dataItem["name"] = data.users[i].name;
    					}
    					dataItem["guid"] = data.users[i].guid;
						dataItem["usertype"] = data.users[i].usertype;
						treedata.push(dataItem);
    				}
    				
    				viewModel.userDataTable.setSimpleData(treedata,{unSelect:true});
    			}
    		}
    	...
    	if(nodes ==null || nodes.length == 0){
    		...
    		if(nodes[0].isParent){
		    	...
		    	$.ajax({
		    		...
		    		success: function(data){
		    			if(data != null){
		    			viewModel.roleDataTable.setSimpleData(data.addroles);
 ...
 // 查詢下一個
    ...
    viewModel.saveDataRightReltion = function(){
    	...
    	//选中角色的树的所有id
//    	var roleTree = $.fn.zTree.getZTreeObj("selectRoleTree");
//    	var roleSelectTree = roleTree.transformToArray(roleTree.getNodes());
