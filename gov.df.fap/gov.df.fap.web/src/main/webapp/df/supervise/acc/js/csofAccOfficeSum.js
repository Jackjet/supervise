...
require(
		...
		function($, ko, ncrd, csscsof) {
			...
			var officeSumURL = '/df/csofacc/saveAccSupOffice.do';//台账办汇总
			var officeUndoSumURL = '/df/csofacc/deleteAccSupOffice.do';//台账办撤销汇总
			var doPayWorkFlowURL= '/df/workflow/work.do';//工作流
			var getDtailAccSupEndURL = '/df/csofacc/getAccSupDepBysetid.do';//点击底级获取明细
			var updateAccSupOfficeURL = '/df/csofacc/updateAccSupOffice.do';//办汇总后修改保存数据
	        	...
	        	if(nodes.length > 0) {
	        		if(nodes[0].LEVEL_NUM == "3"){
	    				...
	    				if(num == nodeChildFlag){
			        				...
			        				for(var i=0;i<workData.length;i++){
			        					if(monthlyerror > 0){
				        					...
				        					var data = {
				        		        		    ...
				        		        		    "parent_id":nodes[0].PARENT_ID,
				        		        		    "book_id":BOOK_ID,
				        		        		    "acc_sup_office_id":workData[i].ACC_SUP_OFFICE_ID,
				        		        		    "op_type":op_type,
				        		        		    "op_name":op_name,
				        		        		    "menu_id":menuId,
				        		        		    "billtype_code":sumBillTypeCode,
	    					...
	    					ip.ipInfoJump("该月份未全部待汇总，不可汇总", 'info');
	    				}
	        		}else{
	        			ip.ipInfoJump("请选择月汇总月份", 'info');
	        		}
	        	}else{
	        		ip.ipInfoJump("请选择汇总月份", 'info');
	        	}	        		        		        	
	        };
	        
	        //撤销汇总officeUndoSumURL
	        UndoSummary = function(id) {
	        	...
	        	if(nodes.length > 0) {
	        		if(nodes[0].LEVEL_NUM == "3"){
	        			...
	        			ip.ipInfoJump("该月份未全部月结，不可撤销汇总", 'info');
	        		}
	        	}else{
	        ...
	        workFlowReturn = function(id){
	        	...
	        	if(nodes.length > 0) {
	    				...
	    				if(nodes[0].STATUS_CODE2 == "11" && nodes[0].IS_LEAF == "1"){
	    					...
	    					ip.ipInfoJump("该月份未全部待汇总，不可退回", 'info');
	    				}
	        ...
	        viewModel.modifySum = function() {
				...
				if(verifyInputView(aim,noVerifyRid)) {
					...
					var data = {
						...
						$.ajax({
				            type: 'get',
				            url: updateAccSupOfficeURL + "?tokenid=" + tokenid + "&ajax=noCache",
				            data: data,
				            dataType: 'JSON',
				            async: true,
				            success: function (result) {
	        	...
	        	if(nodes.length > 0) {
	        		if(nodes[0].LEVEL_NUM == "3"){
	    				...
	    				if(num == nodeChildFlag){
	    					...
	    					ip.ipInfoJump("该月份未汇总完成，不可提交", 'info');
	    				}
	        		}else{
	        			ip.ipInfoJump("请选择提交月份", 'info');
	        		}
	        	}else{
	        		ip.ipInfoJump("请选择提交月份", 'info');
	        	}
