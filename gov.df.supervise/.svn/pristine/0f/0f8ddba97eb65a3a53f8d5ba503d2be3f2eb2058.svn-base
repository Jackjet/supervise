...
public class CommonBo implements CommonService {
  ...
  public boolean saveWorkFlow(Map workMap, String[] ids) {
    ...
    String opinion = "";
    if(null!=workMap.get("opinion")){
    	opinion = workMap.get("opinion").toString();
    }
    int num = 0;
    if (ids.length > 0) {
      for (int i = 0; i < ids.length; i++) {
        String entity_id = ids[i].replace("'", "");
        boolean flag = workflow.doWorkFlow(menu_id, entity_id, billtype_code, op_type, op_name, opinion);
        if (flag) {
          num++;
        }
      }
  ...
  public boolean doWorkFlow(Map workMap) {
    ...
    String op_type = workMap.get("op_type").toString();
    String op_name = workMap.get("op_name").toString();
    String entity_id = workMap.get("entity_id").toString();
    String billtype_code = workMap.get("billtype_code").toString();
    String opinion = null!=workMap.get("opinion")?workMap.get("opinion").toString():"";
    return workflow.doWorkFlow(menu_id, entity_id, billtype_code, op_type, op_name, opinion);
  }
  ...
  public List getWorkData(String menu_id, String status, String billtype_code, Map conditionMap, String pageInfo) {
    ...
    String from_node_id = res.get("FROM_NODE_ID") != null ? res.get("FROM_NODE_ID").toString() : "";
  ...
  public String getWorkQuerySql(Map paramMap) {
    ...
    if (null != node_type && node_type.equals("2")&&status.equals("1")) {
    ...
    if(billtype_code.equals("131")){
        if (org_type.equals("106")) {   //106为业务处室则 有数据权限及用户权限
          ...
          userSql = " and exists ( select 1 from  CSOF_S_SUPTYPE_DEP  s" + " where s.sup_type_id=a.sup_type_id "
            ...
            +" and exists ( select ele_value "
	  		...
	  		+ " and ((d.ele_code='CSOF_SUPTYPE'  and (d.ele_value='#' or d.ele_value=a.sup_type_code)) "
	  		+ ") and r.user_id= '" + user_id + "'" 
	  		+ ")";
        }
    }else if(billtype_code.equals("132")){
          orgSql = " and exists (" + "select 1 from sys_user_org o " + " where 1=1" + " and o.org_id=a." + org
                  + " and o.user_id= '" + user_id + "')";
          
          userSql = " and exists ( select 1 from  CSOF_S_SUPTYPE_DEP  s" 
                  	+ " where s.sup_type_id=a.sup_type_id "
        		  	+ "and s.oid=a.oid " 
                  	+ "and s.dep_id=(select  m.belong_org  from sys_usermanage  m where m.user_id='"
        		  	+ user_id + "')"
        		  	+ ")"
        		  	+" and exists ( select ele_value "
        		  		+ "from sys_right_group_detail d,sys_right_group g,sys_user_rule r  where 1=1 "
        ...
        userSql = " and exists (select ele_value "
    ...
    sql.append(statusSql).append(orgSql).append(userSql);
    return sql.toString();
  }
