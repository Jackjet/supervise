...
public class SupBo implements SupService {
  ...
  public int deleteSup(String ids, String key) {
    ...
    commonService.deleteDataList("csof_sup_report", " and sid in (" + value + ")");
    commonService.deleteDataList("csof_attachment", " and entity_id in (" + value + ")");
  ...
  public int validSup(String ids){
	  String condition = "";
	    ...
	    if (!ids.equals("")) {
	    	...
	    	value = value.substring(0, value.lastIndexOf(","));
	      condition = " and  chr_id  in (" + value + ")";
	    }
	    String sql ="update csof_e_sup set is_valid = 0 where 1=1 "+condition;
