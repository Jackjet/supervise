...
public class TaskBO implements TaskService {
  ...
  static String[] bdq_name = new String[] { "" }; //不定期

  static String[] ycx = new String[] { "--------" }; //一次性

  static String[] ycx_name = new String[] { "" }; //一次性

  static String[] year = new String[] { "" }; //一次性

  static String[] year_name = new String[] { set_year }; //一次性
  ...
  static {
    {
      ...
      cycleMapName.put("9", ycx_name);
      cycleMapName.put("0", bdq_name);
      cycleMapName.put("1", year_name);
  ...
  public void setDepAssign(Map map) {
    String upDep = " update csof_task_dep set remark='" + map.get("REMARK") + "' , plan_cost=0, end_date='" + map.get("END_DATE") + "' ," + " assign_user='" + map.get("ASSIGN_USER")
  ...
  */
  ...
  public int saveTask(Map<String, String> supMap, List objList, List agencyList, List reportList) {
    ...
    Map depSumMap = new HashMap<String,Integer>();      //用来计算处室所监管的单位数量
    ...
    if (null != supMap.get("IS_ALLOBJ") && supMap.get("IS_ALLOBJ").equals("0") && agencyList.size() > 0) {
      ...
      for (int i = 0; i < agencyList.size(); i++) {
        ...
        if (type.equals("AGENCY") && is_leaf.equals("1")) {
          sup_num++;
          String depSum=depSumMap.get(obj.get("DEP_ID"))!=null?depSumMap.get(obj.get("DEP_ID")).toString():"0";
          depSumMap.put(obj.get("DEP_ID"), Integer.parseInt(depSum)+1);
        }
      ...
      dao.executeBatchBySql(agencySqlList);
    	...
    	//获取监管单位数量
    	List agencyAllList=getData("vw_csof_e_agency", " and IS_LEAF=1 and ENABLED=1 and OID='"+oid+"'");
    	sup_num = agencyAllList.size();
      
    	for(int i=0;i<agencyAllList.size();i++){
    		Map agency = (Map)agencyAllList.get(i);
    		String depSum=depSumMap.get(agency.get("dep_id"))!=null?depSumMap.get(agency.get("dep_id")).toString():"0";
            depSumMap.put(agency.get("dep_id"), Integer.parseInt(depSum)+1);
    	}
    }
    ...
    for (int i = 0; i < objList.size(); i++) {
      ...
      depMap.put("DEP_TASK_NAME", supMap.get("SUP_NAME") + "(" + dep_name + ")");
      depMap.put("SUP_NAME", supMap.get("SUP_NAME"));
      depMap.put("DEP_ID", dep_id);
      depMap.put("DEP_CODE", dep_code);
      depMap.put("DEP_NAME", dep_name);
      depMap.put("OID", oid);
      depMap.put("SUP_NUM", depSumMap.get(dep_id)!=null?depSumMap.get(dep_id).toString():"0");
  ...
  public int updateTaskDep(String data) {
    ...
    if (depList.size() > 0) {
      ...
      String sql = "update csof_task_dep  set  remark='" + depMap.get("REMARK") + "' , plan_cost="
        + depMap.get("PLAN_COST") + " , end_date ='" + depMap.get("END_DATE") + "'" +" where ID='" + depMap.get("ID") + "'";
      
      String  userSql="update csof_task_user set remark = '"+depMap.get("REMARK")+"' where dep_task_id ='"+depMap.get("ID")+"'";
      
      dao.executeBySql(userSql);
