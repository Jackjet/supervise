...
public class SummaryBo implements SummaryService {
	...
	public int saveSummary(Map sumData) {
	    ...
	    String bill_id = "";
	    if(null!=sumData.get("BILL_ID")|| null!=sumData.get("bill_id")){
		    bill_id = null!=sumData.get("BILL_ID")?sumData.get("BILL_ID").toString():sumData.get("bill_id").toString();
	    }else{
		    bill_id = null!=sumData.get("ID")?sumData.get("ID").toString():sumData.get("id").toString();
	    }
	...
	public int deleteSummary(String ids) {
		...
		if(!ids.contains("'")){
			...
			ids = v.substring(0, v.lastIndexOf(","));
		}
	...
	public int updateSummary(Map summaryData) {
		...
		commonService.deleteDataList("CSOF_TASK_SUMMARY",  "  and id ='"+summary_id+"' ");
